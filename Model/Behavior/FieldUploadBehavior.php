...
class FieldUploadBehavior extends ModelBehavior {
	...
	public function afterSave(Model $Model, $created, $options = []) {
		$this->_startUploadQueue($Model);	// Uploads anything found in beforeSave
		$this->_startDeleteQueue($Model);	// Deletes anything marked for deletion
 ...
 **/
	private function _startDeleteQueue(Model $Model) {
		if (!empty($this->_deleteQueue[$Model->alias][$Model->id])):
			$fields = array_keys($this->_deleteQueue[$Model->alias][$Model->id]);
			$this->_deleteFields($Model, $Model->id, $fields);
			unset($this->_deleteQueue[$Model->alias][$Model->id]);
			return true;
		endif;
		return null;
	}
